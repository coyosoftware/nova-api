RSpec.describe Nova::API::Resource::DirectReceivable do
  before(:all) do
  ...
  describe 'attributes' do
    ...
    it { is_expected.to have_attribute(:installments, Dry::Types['strict.array'].of(Nova::API::Resource::DirectReceivable::Installment).optional) }

    context 'installments' do
      subject { described_class::Installment }

      it { is_expected.to have_attribute(:card_tax_id, Dry::Types['coercible.integer'].optional) }
  ...
  describe '.endpoint' do
    it 'returns the save direct receivables endpoint' do
      expect(described_class.endpoint).to eq('/api/receivables/save_direct')
    end
  end
  ...
  describe '.create' do
    ...
    let(:installments) do
      [
        {
          card_tax_id: card_tax_id, current_asset_id: current_asset_id, document_number: '123ABC', document_type: document_type,
          due_date: '2021-01-16', gross_value: gross_value, value: total_value }
      ]
    end
    let(:parameters) do
      {
        ...
        gross_value: gross_value, installments: installments, identifier: '#123', third_party_id: third_party_id, total_value: total_value
      }
    end
    ...
    it 'issues a post to the receivable save direct endpoint' do

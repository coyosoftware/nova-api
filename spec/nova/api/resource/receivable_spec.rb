RSpec.describe Nova::API::Resource::Receivable do
  ...
  describe '.find' do
    ...
    it 'issues a get to the receivable show endpoint' do
    ...
    context 'with a successful response' do
      ...
      it 'returns the record' do
        ...
        expect(response.record).to be_a(Nova::API::Resource::Receivable)

        expect(response.record.additional_information).to eq(data[:additional_information])

        expect(response.record.apportionments).to all(be_a(Nova::API::Resource::Receivable::Apportionment))
        ...
        expect(response.record.financial_accounts).to all(be_a(Nova::API::Resource::Receivable::FinancialAccount))
    ...
    context 'with an error response' do
      ...
      before { stub_request(:get, "#{described_class.base_url}#{described_class.endpoint}/#{id}").to_return(status: 404, body: JSON.generate({ errors: errors })) }
